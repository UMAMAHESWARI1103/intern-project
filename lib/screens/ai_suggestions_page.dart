...
// ✅ TWO TABS:
// TAB 1 — 📍 Near Me   : vehicle picker → Temples Near You + Good Time To Visit
// TAB 2 — 🧠 For You   : ML Recommended For You + Popular Temples Today
...
class _AiSuggestionsPageState extends State<AiSuggestionsPage>
  ...
  static const Map<String, double> _speedKmh = {
    'walk': 4, 'bike': 25, 'car': 20, 'bus': 15,
  };
  static const Map<String, String> _vehicleLabel = {
    'walk': 'Walking', 'bike': 'Bike', 'car': 'Car', 'bus': 'Bus',
  };
  ...
  Future<void> _runGPS() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _gpsError   = 'Could not load temples. Is backend running?';
          _gpsLoading = false;
        });
      }
      return;
    ...
    final byDist  = List<_TempleResult>.from(results)
      ..sort((a, b) => a.dist.compareTo(b.dist));
    final byScore = List<_TempleResult>.from(results)
      ..sort((a, b) => b.score.compareTo(a.score));
    final goodTime = byScore
        .where((r) => r.isOpen && r.crowd != 'very_high')
        .take(6)
        .toList();
  ...
  Widget _forYouTab() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_mlLoading)
        _loadingBox('Loading your recommendations...', _purple),
  ...
  Widget _buildGoodTimeCard(_TempleResult t) => Container(
    ...
    child: Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 3),
        if (t.location.isNotEmpty)
          Text(t.location, style: TextStyle(fontSize: 11, color: Colors.grey.shade400),
              maxLines: 1, overflow: TextOverflow.ellipsis),
        const Spacer(),
  ...
  Widget _buildMLCard(Map<String, dynamic> t, int rank, {bool forYou = false}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(
          color: isTop ? _orange.withValues(alpha: 0.12) : Colors.black.withValues(alpha: 0.07),
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Stack(clipBehavior: Clip.none, children: [
            Container(
              ...
              child: Center(child: Text(forYou ? '🛕' : (rank == 1 ? '🔥' : '🛕'),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 3),
            if (loc.isNotEmpty)
              Text('📍 $loc', style: TextStyle(fontSize: 12, color: Colors.grey.shade500),
                  maxLines: 1, overflow: TextOverflow.ellipsis),
            if (deity.isNotEmpty) ...[
              const SizedBox(height: 2),
              Text('🕉️ $deity',
            ...
            if (festStr.isNotEmpty) ...[
              const SizedBox(height: 2),
              Text(festStr, style: TextStyle(fontSize: 11, color: Colors.grey.shade500),
                  maxLines: 1, overflow: TextOverflow.ellipsis),
            ],
            const SizedBox(height: 8),
