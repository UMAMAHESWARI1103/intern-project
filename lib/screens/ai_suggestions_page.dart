...
class _AiSuggestionsPageState extends State<AiSuggestionsPage>
  ...
  Future<void> _runGPS() async {
    setState(() { _gpsLoading = true; _gpsError = null; _nearbyList = []; _goodTimeList = []; });
    Position pos;
    try {
    ...
    try {
      ...
      temples = raw.map((e) => Map<String, dynamic>.from(e as Map))
          .where((t) => (t['lat'] ?? t['latitude']) != null && (t['lon'] ?? t['longitude']) != null)
          .toList();
    } catch (_) {
      if (mounted) setState(() { _gpsError = 'Could not load temples.'; _gpsLoading = false; });
      return;
    }
    ...
    for (final t in temples) {
      ...
      results.add(_TempleResult(
        name: (t['name'] ?? 'Temple').toString(), location: (t['location'] ?? '').toString(),
        deity: (t['deity'] ?? '').toString(), dist: dist, travelMin: tr,
        crowd: crowd, isOpen: session.isOpen, session: session.label,
        minsLeft: session.minsLeft, score: _score(dist, crowd, session.isOpen, session.minsLeft),
      ));
    }
  ...
  Future<void> _loadML() async {
    ...
    try {
      ...
      // STEP 1: Try to decode email directly from JWT token
      ...
      // STEP 2: Try SharedPreferences with all possible keys
      if (email.isEmpty) {
      ...
      // STEP 3: Fallback — call profile API and check all possible fields
      if (email.isEmpty) {
      ...
      final uri = Uri.parse('${ApiService.baseUrl}/ml-recommendations/${Uri.encodeComponent(email)}');
      ...
      if (res.statusCode == 200) {
        ...
        if (data['success'] == true) {
          final forYouRaw  = (data['forYou']  as List? ?? []).map((e) => Map<String, dynamic>.from(e as Map)).toList();
          final popularRaw = (data['popular'] as List? ?? []).map((e) => Map<String, dynamic>.from(e as Map)).toList();

          if (forYouRaw.isEmpty && popularRaw.isEmpty) {
            final temples = (data['temples'] as List? ?? []).map((e) => Map<String, dynamic>.from(e as Map)).toList();
            if (mounted) setState(() { _popularList = temples.take(8).toList(); _mlLoading = false; });
            return;
          }
          if (mounted) setState(() { _forYouList = forYouRaw; _popularList = popularRaw; _mlLoading = false; });
          return;
        }
      }
      if (mounted) setState(() { _mlLoading = false; _mlError = 'Could not load recommendations. Try again later.'; });
    } catch (_) {
      if (mounted) setState(() { _mlLoading = false; _mlError = 'Could not load recommendations. Try again later.'; });
    }
  }
  ...
  int _parseHour(String s) {
    try {
      ...
      if (!t.contains('AM') && !t.contains('PM')) return int.parse(t.split(':')[0]);
      final isPM = t.contains('PM');
      int h = int.parse(t.replaceAll('AM', '').replaceAll('PM', '').trim().split(':')[0]);
      if (isPM && h != 12) h += 12;
      if (!isPM && h == 12) h = 0;
      return h;
    } catch (_) { return -1; }
  ...
  _Session _checkSession(int o, int c, int r, int f, int tr, DateTime now) {
    ...
    if (o >= 0 && c >= 0 && arrH >= o && arrH < c) return _Session(true,  c * 60 - (nm + tr), 'Morning session open');
    if (r >= 0 && f >= 0 && arrH >= r && arrH < f) return _Session(true,  f * 60 - (nm + tr), 'Evening session open');
  ...
  String _ds(double km) => km < 1 ? '${(km * 1000).round()} m' : '${km.toStringAsFixed(1)} km';

  Color _crowdColor(String c) => const {
    'low': Colors.green, 'medium': Colors.orange, 'high': Colors.red, 'very_high': Colors.purple,
  }[c] ?? Colors.grey;

  String _crowdText(String c) => const {
    'low': 'Less crowd', 'medium': 'Moderate crowd', 'high': 'Busy', 'very_high': 'Very crowded',
  }[c] ?? '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Discover Temples', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        actions: [
          IconButton(
      ...
      body: TabBarView(controller: _tabCtrl, children: [_nearMeTab(), _forYouTab()]),
    );
  }
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Row(children: [
            ...
            Text('How are you travelling?', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
          ]),
          const SizedBox(height: 14),
          Row(children: ['walk', 'bike', 'car', 'bus'].map((v) {
            final sel = _vehicle == v;
            return Expanded(child: GestureDetector(
              onTap: () => setState(() { _vehicle = v; _gpsDone = false; _nearbyList = []; _goodTimeList = []; }),
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: sel ? [BoxShadow(color: _orange.withValues(alpha: 0.3), blurRadius: 8, offset: const Offset(0, 3))] : [],
                ),
                child: Column(children: [
                  Icon(_vehicleIcon[v]!, color: sel ? Colors.white : Colors.grey.shade500, size: 22),
                  const SizedBox(height: 4),
                  Text({'walk': 'Walk', 'bike': 'Bike', 'car': 'Car', 'bus': 'Bus'}[v]!,
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(
            ...
            child: _gpsLoading
                ...
                : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    Text(_gpsDone ? '🔄 Search Again' : '✨ Find Temples Near Me',
      ...
      if (_gpsDone && !_gpsLoading) ...[
        ...
        if (_goodTimeList.isNotEmpty) ...[
          ...
          SizedBox(height: 215, child: ListView.builder(
            scrollDirection: Axis.horizontal, itemCount: _goodTimeList.length,
    ...
    child: Padding(padding: const EdgeInsets.all(16),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Column(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: t.isOpen ? Colors.green.shade300 : Colors.grey.shade300),
            ),
            child: Text(t.isOpen ? 'Open' : 'Closed',
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (t.location.isNotEmpty) Text('📍 ${t.location}', style: TextStyle(fontSize: 12, color: Colors.grey.shade500), maxLines: 1, overflow: TextOverflow.ellipsis),
          if (t.deity.isNotEmpty) ...[const SizedBox(height: 2), Text('🕉️ ${t.deity}', style: const TextStyle(fontSize: 11, color: _orange, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis)],
          const SizedBox(height: 8),
          Text(_nearbyReason(t), style: TextStyle(fontSize: 12, color: Colors.grey.shade700, height: 1.4), maxLines: 2),
          const SizedBox(height: 10),
          Wrap(spacing: 6, runSpacing: 6, children: [
            _chip('${_ds(t.dist)} away', Colors.teal), _chip('${t.travelMin} min', Colors.blue),
  ...
  Widget _buildGoodTimeCard(_TempleResult t) => Container(
    width: 178, margin: const EdgeInsets.only(right: 12, bottom: 4),
    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18),
    ...
    child: Padding(padding: const EdgeInsets.all(14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        Text(t.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13), maxLines: 2, overflow: TextOverflow.ellipsis),
        const SizedBox(height: 3),
        if (t.location.isNotEmpty) Text(t.location, style: TextStyle(fontSize: 11, color: Colors.grey.shade400), maxLines: 1, overflow: TextOverflow.ellipsis),
        const Spacer(),
        Text(_goodTimeReason(t), style: TextStyle(fontSize: 11, color: Colors.green.shade700, height: 1.3), maxLines: 2),
        const SizedBox(height: 8),
        Wrap(spacing: 4, runSpacing: 4, children: [_miniChip(_ds(t.dist), Colors.teal), _miniChip(_crowdText(t.crowd), _crowdColor(t.crowd))]),
      ]),
    ),
  );
  ...
  Widget _buildMLCard(Map<String, dynamic> t, int rank, {bool forYou = false}) {
    ...
    final festStr = (fests is List && fests.isNotEmpty) ? '✨ Popular during ${fests.first}' : '';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white, borderRadius: BorderRadius.circular(18),
        border: isTop ? Border.all(color: _orange.withValues(alpha: 0.5), width: 1.5) : Border.all(color: Colors.grey.shade100),
        boxShadow: [BoxShadow(color: isTop ? _orange.withValues(alpha: 0.12) : Colors.black.withValues(alpha: 0.07), blurRadius: isTop ? 14 : 8, offset: const Offset(0, 3))],
      ),
      child: Padding(padding: const EdgeInsets.all(16),
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Stack(clipBehavior: Clip.none, children: [
            Container(width: 56, height: 56,
                decoration: BoxDecoration(color: forYou ? _orange.withValues(alpha: 0.12) : Colors.red.shade50, borderRadius: BorderRadius.circular(12)),
                child: Center(child: Text(forYou ? '🛕' : (rank == 1 ? '🔥' : '🛕'), style: const TextStyle(fontSize: 26)))),
            if (isTop) Positioned(top: -5, right: -5,
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 3),
            if (loc.isNotEmpty) Text('📍 $loc', style: TextStyle(fontSize: 12, color: Colors.grey.shade500), maxLines: 1, overflow: TextOverflow.ellipsis),
            if (deity.isNotEmpty) ...[const SizedBox(height: 2), Text('🕉️ $deity', style: TextStyle(fontSize: 11, color: accentColor, fontWeight: FontWeight.w600))],
            if (festStr.isNotEmpty) ...[const SizedBox(height: 2), Text(festStr, style: TextStyle(fontSize: 11, color: Colors.grey.shade500), maxLines: 1, overflow: TextOverflow.ellipsis)],
            const SizedBox(height: 8),
            Text(forYou ? _forYouReason(t, rank) : _popularReason(t),
                style: TextStyle(fontSize: 12, color: Colors.grey.shade700, height: 1.4), maxLines: 2),
          ])),
        ]),
      ),
    );
  ...
  Widget _sectionTitle(String title, String sub) => Padding(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: _dark)),
  ...
  Widget _errorBox(String msg) => Container(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Text(msg, style: const TextStyle(fontSize: 13, color: Colors.brown))),
    ]),
  );
  ...
  Widget _loadingBox(String msg, Color color) => Padding(
    ...
    child: Column(children: [
      CircularProgressIndicator(color: color, strokeWidth: 3), const SizedBox(height: 16),
  ...
  Widget _emptyBox(String emoji, String title, String sub) => Container(
    margin: const EdgeInsets.only(top: 20), padding: const EdgeInsets.all(28),
    ...
    child: Column(children: [
      Text(emoji, style: const TextStyle(fontSize: 40)), const SizedBox(height: 12),
      Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)), const SizedBox(height: 6),
      Text(sub, textAlign: TextAlign.center, style: TextStyle(color: Colors.grey.shade600, fontSize: 13, height: 1.5)),
    ]),
  );
...
class _Session {
  final bool isOpen; final int minsLeft; final String label;
...
class _TempleResult {
  ...
  const _TempleResult({
    ...
    required this.isOpen, required this.session, required this.score, required this.minsLeft,
  });
}