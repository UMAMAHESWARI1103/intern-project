...
class _HomamBookingPageState extends State<HomamBookingPage>
  ...
  static const List<String> _stepLabels = ['Venue','Details','Homam','Date & Time','Pandit','Payment'];
  static const List<IconData> _stepIcons = [
    Icons.location_on_outlined, Icons.person_outline,
    Icons.local_fire_department_outlined, Icons.calendar_today_outlined,
    Icons.self_improvement_outlined, Icons.payment_outlined,
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: [
        _buildHeader(),
        _buildStepBar(),
        Expanded(child: SlideTransition(position: _slideAnim, child: _buildCurrentStep())),
      ]),
    );
  }
  ...
  Widget _buildHeader() => Container(
    ...
    child: Row(children: [
      GestureDetector(
        onTap: () => _step == 0 ? Navigator.pop(context) : _goToStep(_step - 1),
        child: Container(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                color: active ? Colors.white
                    : done  ? Colors.white.withValues(alpha: 0.9)
  ...
  Widget _buildVenueStep() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _nextButton(label: 'Continue to Details', onTap: () {
  ...
  Widget _buildDetailsStep() => SingleChildScrollView(
    ...
    child: Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 32),
        _nextButton(label: 'Continue to Homam Selection', onTap: () {
  ...
  Widget _buildHomamStep() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(
                    ...
                    children: [
                      ...
                      Text('₹${(_homamInfo[h]!['price']! as double).toStringAsFixed(0)}',
      ...
      const SizedBox(height: 32),
      _nextButton(label: 'Continue to Date & Time', onTap: () {
  ...
  Widget _buildDateTimeStep() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    child: Row(children: [
                      ...
                      if (t == '04:30 AM' || t == '05:00 AM') ...[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                              color: Colors.deepOrange, borderRadius: BorderRadius.circular(4)),
      ...
      const SizedBox(height: 32),
      _nextButton(label: 'Find Available Pandits', onTap: () {
        _loadPriests();
        _goToStep(4);
      }),
    ]),
  ...
  // ══════════════════════════════════════════════════════════════
  // STEP 4 — PRIEST  ✅ DROPDOWN — itemHeight:56, 2-line items only
  // ══════════════════════════════════════════════════════════════
  Widget _buildPriestStep() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      // Empty
        ...
        _card(child: Padding(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 12),
            const Text('No specific pandits found', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 6),
      ...
      // ── Dropdown — strict 2-line rows, itemHeight: 56 ─────────
      else ...[
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                isExpanded: true,
                // ✅ KEY FIX: explicit item height so Flutter never overflows
                itemHeight: 56,
                hint: const Text('Select a pandit', style: TextStyle(color: Colors.grey)),
                icon: const Icon(Icons.keyboard_arrow_down, color: _primary),

                // Closed state — single line
                selectedItemBuilder: (_) => _priests.map((p) {
                  final pr = Map<String, dynamic>.from(p as Map);
                  return Align(
                    alignment: Alignment.centerLeft,
                    child: Row(children: [
                      _priestAvatar(pr, size: 30, selected: true),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(pr['name'] ?? '',
                ...
                // Open state — fixed SizedBox(height:56), zero overflow
                items: _priests.map((p) {
                  final pr  = Map<String, dynamic>.from(p as Map);
                  final id  = pr['_id'] as String? ?? '';
                  final loc = pr['location'] as String? ?? '';
                  final exp = pr['experience'] ?? 0;
                  final rat = pr['rating'] ?? 0;
                  return DropdownMenuItem<String>(
                    ...
                    child: SizedBox(
                      ...
                      child: Row(
                        ...
                        children: [
                          _priestAvatar(pr, size: 36, selected: false),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Column(
        ...
        // Selected pandit detail card below dropdown
      ...
      const SizedBox(height: 32),
      _nextButton(label: 'Continue to Payment', onTap: () {
  ...
  // ── Selected pandit card (shown below dropdown) ────────────────
  Widget _buildSelectedPriestCard(Map<String, dynamic> pr) {
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _priestAvatar(pr, size: 50, selected: true),
        const SizedBox(width: 12),
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Wrap(spacing: 5, runSpacing: 4,
                children: specs.take(3).map((s) => Container(
                  ...
                  child: Text(s.toString(),
                      style: const TextStyle(
                          fontSize: 9, color: _primary, fontWeight: FontWeight.w600)),
                )).toList(),
              ),
            ],
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_gotraCtrl.text.isNotEmpty) _summaryRow('🌿 Gotra', _gotraCtrl.text),
          if (_nakshatraCtrl.text.isNotEmpty) _summaryRow('⭐ Nakshatra', _nakshatraCtrl.text),
          const SizedBox(height: 12),
          const Divider(),
          const SizedBox(height: 12),
  ...
  void _initiatePayment() {
    ...
    _pendingBookingData = {
      ...
      'templeName':   _venueType == 'temple'
          ? (_selectedTemple?.name ?? '') : 'Home - ${_nameCtrl.text.trim()}',
  ...
  void _onPaymentSuccess(PaymentSuccessResponse response) async {
    ...
    try { await ApiService.saveHomamBooking(_pendingBookingData!); }
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ...points.map((pt) => Padding(
              ...
              child: Row(children: [
                ...
                Text(pt, style: TextStyle(fontSize: 11,
  ...
  Widget _nextButton({required String label, required VoidCallback onTap}) => SizedBox(
    width: double.infinity, height: 52,
    child: ElevatedButton(
      onPressed: onTap,
      style: ElevatedButton.styleFrom(
        backgroundColor: _primary, foregroundColor: Colors.white,
        elevation: 4, shadowColor: _primary.withValues(alpha: 0.4),
